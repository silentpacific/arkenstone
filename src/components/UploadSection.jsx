...
const UploadSection = () => {
  ...
  const startProcessing = async (file) => {
    ...
    try {
      ...
      console.log('Sending request to process-jewelry...');
      ...
      setProcessingSteps(prev => [...prev, { 
        text: `Enhancement failed: ${error.message}`, 
        status: 'error' 
      }]);
      
      // Show error alert instead of silent refresh
                  ...
                  {Object.entries(processingResult.apiStats).map(([service, stats]) => (
                    <div key={service} className="bg-white/5 rounded-lg p-3">
                      ...
                      <div className="space-y-1 text-blue-200">
                        ...
                        {stats.lastError && <p className="text-red-300">Error: {stats.lastError}</p>}
                      </div>
                    </div>
                  ))}
                </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 flex items-center justify-center p-4">
        <div className="max-w-2xl w-full">
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold text-white mb-4">🏔️ Arkenstone</h1>
            <p className="text-red-400 text-xl">❌ Enhancement Failed</p>
          </div>

          <div className="bg-white/5 backdrop-blur-sm rounded-xl p-8 border border-red-500/20">
            <div className="text-center mb-6">
              <img 
                src={uploadedImage.preview} 
                alt="Failed" 
            ...
            <div className="space-y-3 mb-6">
              {processingSteps.map((step, index) => (
                <div key={index} className="flex items-center space-x-3">
                  ...
                  {step.status === 'complete' && <span className="text-green-400 ml-auto">✓</span>}
                  {step.status === 'error' && <span className="text-red-400 ml-auto">✗</span>}
                </div>
              ))}
            </div>
